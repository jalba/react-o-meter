...
class Meter extends Component {
	...
	render() {
		...
		const needleStyles = this.validateProps() ? Object.assign(styles.needle, transform) : styles.needle;
		return (
            <div style={ styles.container }>
                <h2 style={ styles.value } >{ unit }{ value }</h2>
                ...
                <span style={ styles.min } >{ unit }{ min }</span>
                <span style={ styles.max } >{ unit }{ max }</span>
            </div>
		);
	}
}
